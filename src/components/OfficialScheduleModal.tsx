...
import { X, Download, ZoomIn, ZoomOut, CheckCircle2, FileImage, RotateCcw, Loader2 } from 'lucide-react';
...
export const OfficialScheduleModal: React.FC<OfficialScheduleModalProps> = memo(({
  ...
  const handleDownload = useCallback(() => {
    ...
    if (svgElement) {
      try {
          ...
          if (ctx) {
            ...
            canvas.toBlob((pngBlob) => {
              if (pngBlob) {
                ...
                link.href = pngUrl;
                link.download = document.downloadFileName || 'FEE_Menouf_Official_Schedule.png';
        ...
        const link = window.document.createElement('a');
        link.href = url;
        link.download = 'FEE_Menouf_Official_Schedule.svg';
        window.document.body.appendChild(link);
        link.click();
        window.document.body.removeChild(link);
  ...
  return (
    <BottomSheet isOpen={isOpen && !!document} onClose={onClose} id="official-schedule-sheet" maxWidthClass="max-w-2xl">
      {document && (
        <>
          {/* Modal Header */}
          ...
          {/* Action Controls Bar */}
          <div className="px-4 py-2.5 bg-slate-100/90 border-b border-slate-200 flex items-center justify-between text-xs gap-2 flex-wrap">
            <div className="flex items-center gap-1.5">
              <button
                type="button"
                onClick={handleZoomIn}
                className="p-1.5 px-2 bg-white rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50 active:scale-95 transition-all shadow-2xs flex items-center gap-1 font-bold"
                title="تكبير"
              >
                <ZoomIn size={14} />
                <span>تكبير</span>
              ...
              <button
                ...
                className="p-1.5 px-2 bg-white rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 active:scale-95 text-xs font-semibold flex items-center gap-1"
                title="إعادة ضبط"
              >
                <RotateCcw size={12} />
                <span>الافتراضي</span>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-bold flex items-center gap-1.5 transition-all shadow-xs text-xs ${
                isDownloaded
                  ? 'bg-emerald-600 text-white'
                  : 'bg-blue-600 hover:bg-blue-700 text-white active:scale-95 shadow-md shadow-blue-600/20'
              }`}
            >
              {isDownloading ? (
                <>
          ...
          {/* Schedule Sheet Content Viewer with Drag / Pan container */}
          <div className="p-3 bg-slate-200/80 overflow-auto flex-1 flex items-center justify-center min-h-[380px] max-h-[58vh]">
            <div
              style={{
              ...
              className="bg-white p-3.5 rounded-xl shadow-md border border-slate-300 w-full max-w-[680px] shrink-0"
            >
              ...
              >
                ...
                <text x="735" y="52" fill="#94a3b8" fontSize="11" fontWeight="600" textAnchor="end">
                  شؤون التعليم والطلاب • {document.term} {document.academicYear}
                </text>
                ...
                <g transform="translate(30, 442)">
                  ...
                  <text x="215" y="26" fill="#2563eb" fontSize="7.5" fontWeight="bold" textAnchor="middle" opacity="0.8">جامعة المنوفية</text>
                  <text x="215" y="37" fill="#2563eb" fontSize="7" fontWeight="bold" textAnchor="middle" opacity="0.8">كلية الهندسة الإلكترونية</text>
                </g>
              </svg>
            </div>
          </div>
